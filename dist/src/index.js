...
import { relative as relativePath, resolve as resolvePath, normalize as normalizePath } from "path";
...
function updateCollections(metalsmith, collections) {
  const metadata = _extends({}, metalsmith.metadata(), {
    collections
  });
...
export default function (options) {
  ...
  const plugin = function metalsmithWatch(files, metalsmith, cb) {
    ...
    gaze(Object.keys(patterns), _extends({}, options.gaze, {
      ...
      const previousFilesMap = _extends({}, files);
      ...
      const update = () => {
        ...
        pathsToUpdate.some(file => file.match(jsFileRE))) {
          const filesToInvalidate = Object.keys(patterns).reduce((acc, pattern) => {
            return [...acc, ...multimatch(Object.keys(require.cache), `${ resolvePath(metalsmith._directory) }/${ pattern }`)];
          }, []);
        ...
        const patternsToUpdatePattern = Object.keys(patterns).filter(pattern => patterns[pattern] !== true).filter(pattern => multimatch(pathsToUpdate, pattern).length > 0).map(pattern => patterns[pattern]);
