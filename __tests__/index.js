...
function prepareTests(key, testCb, assertionCb, options, beforeWatch) {
  ...
  // watcher might detect dummy file created above
  ...
  test.test("invalidate js cache for deps", (t) => {
    ...
    const folder = prepareTests(
      ...
      () => {
        ...
        const jsfileRequiredception = `${folder}/src/requiredception.js`
        fs.writeFileSync(jsfileRequiredception, "module.exports = function() { return require('./required')(); }")
        const jsfileThatRequire = `${folder}/src/req.js`
        fs.writeFileSync(jsfileThatRequire, "module.exports = function() { return require('./requiredception')(); }")
